...
class stemm_es {
	...
	function stemm($word) {
		...
		if ($suf != '') {
			$pre_suff = stemm_es::endsinArr(mb_substr($rv_txt,0,-mb_strlen($suf)),$pronoun_suf_pre1);
			if ($pre_suff != '') {
				$word = stemm_es::removeAccent(mb_substr($word,0,-mb_strlen($suf)));
			} else {
				$pre_suff = stemm_es::endsinArr(mb_substr($rv_txt,0,-mb_strlen($suf)),$pronoun_suf_pre2);
				if ($pre_suff != '' ||
		...
		if ($word_after0 == $word_after1) {
			...
			if (($suf = stemm_es::endsinArr($rv_txt, array('ya', 'ye', 'yan', 'yen', 'yeron', 'yendo', 'yo', 'yó', 'yas', 'yes', 'yais', 'yamos'))) != '' && (mb_substr($word,-mb_strlen($suf)-1,1) == 'u')) {
				$word = mb_substr($word,0, -mb_strlen($suf));
			}
